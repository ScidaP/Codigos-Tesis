...
for ($i = 1; $i -le 10; $i++) {
    ...
    $locustJob = Start-Job -ScriptBlock {
        ...
        $csvPath = "Resultados\Proxmox\$fechaParam\$valParam-Usuarios\Locust\data"
        $htmlPath = "Resultados\Proxmox\$fechaParam\$valParam-Usuarios\Locust\pagina.html"
        $runtime = 720 + ${valParam}/10
        Write-Host "Corriendo por $runtime segundos" -ForegroundColor Yellow
        locust -f pruebaEstres.py -H http://192.168.100.229:5000 --headless -u $valParam -r 10 `
            --csv $csvPath `
            --html $htmlPath `
            --run-time "${runtime}s"
    ...
    & "C:\Windows\System32\OpenSSH\ssh.exe" -i 'C:\Users\patri\.ssh\id_ed25519' pscida@192.168.100.229 `
        "pruebas-perfomance/benchmarks/BUCLE.sh 'pruebas-perfomance/Resultados/$fecha/$valor-Usuarios' 10"
    if ($LASTEXITCODE -ne 0) {
    ...
    $destino = "E:\Tesis\Codigos_Tesis\Resultados\Proxmox\$fecha\$valor-Usuarios\Datos_CSV"
    if (-not (Test-Path $destino)) {
        Write-Host "Creando carpeta destino: $destino"
        New-Item -ItemType Directory -Path $destino -Force | Out-Null
    }

    & "C:\Windows\System32\OpenSSH\scp.exe" -r "pscida@192.168.100.229:~/pruebas-perfomance/Resultados/$fecha/$valor-Usuarios/*" $destino
